        ...
        {result && (
          <>
            <div className="grid gap-4 md:grid-cols-4">
              <StatCard label="Net value" value={`€ ${result.net_eur.toFixed(2)}`} icon={Euro} hint={`Revenue € ${result.gross_revenue_eur.toFixed(2)} − charge € ${result.charge_cost_eur.toFixed(2)}`} />
              ...
              <StatCard label="Cycles used" value={result.cycles_used.toFixed(2)} icon={RotateCw} hint={`Start SoC ${result.start_soc_pct.toFixed(0)}%${result.start_soc_at ? ` @ ${new Date(result.start_soc_at).toLocaleTimeString()}` : ""}`} />
              <StatCard label="Dispatch rows" value={String(result.dispatch_rows_written)} icon={Play} hint={result.dispatch_rows_written ? "Written as planned arbitrage" : "Dry run / backtest — nothing written"} />
            </div>
