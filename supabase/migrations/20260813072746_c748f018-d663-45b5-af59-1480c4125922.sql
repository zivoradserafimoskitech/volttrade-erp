...
CREATE TABLE IF NOT EXISTS public.gateway_alarms (
  ...
  gateway_alarm_id   bigint NOT NULL UNIQUE,
  device_id          integer,
  gateway_id         integer,
...
DROP POLICY IF EXISTS "staff read gateway_alarms" ON public.gateway_alarms;
...
GRANT ALL ON public.lead_submission_throttle TO service_role;
...
REVOKE ALL ON FUNCTION public.check_lead_throttle(text, int, int, int) FROM public, authenticated, anon;
...
REVOKE ALL ON FUNCTION public.prune_lead_throttle() FROM public, authenticated, anon;
