...
DROP POLICY IF EXISTS "org staff read volume_forecasts" ON public.volume_forecasts;
CREATE POLICY "org staff read volume_forecasts" ON public.volume_forecasts
  FOR SELECT TO authenticated USING (public.is_staff());
DROP POLICY IF EXISTS "bal write volume_forecasts" ON public.volume_forecasts;
...
DROP POLICY IF EXISTS "org staff read api log" ON public.external_api_log;
...
DROP POLICY IF EXISTS "staff read regulatory_charges" ON public.regulatory_charges;
...
DROP POLICY IF EXISTS "staff read public_holidays" ON public.public_holidays;
